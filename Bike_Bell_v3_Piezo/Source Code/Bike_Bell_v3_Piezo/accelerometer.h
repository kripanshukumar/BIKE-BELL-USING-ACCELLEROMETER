void mpu_read(void *parameter) {
  while (1) {                                           // Infinite While loop
    ...
    if (netAcc >= 10000) {                              // Avoids false data
      ...
      if (variance >= sensitivity) {                            // threshold for the lower limit of the variace ( basically sets the sensitivity level)
        vol = float(variance) / 5000;                   // Calculate the volume from the vibration gathered from the accelerometer
